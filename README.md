...
本工程由 [`existyay/Auto-H5000M-BIN`](https://github.com/existyay/Auto-H5000M-BIN)
（基于 ImmortalWrt 的编译脚手架）迁移而来：流水线、命令行参数、`ENABLE_*` 开关命名
和产物布局都刻意保持熟悉，改变的是**上游源码**和**设备功能栈**。

| | 参考工程 Auto-H5000M-BIN | 本工程 |
| --- | --- | --- |
| 上游 | `padavanonly/immortalwrt-mt798x-24.10`（第三方分支） | `openwrt/openwrt` **main（SNAPSHOT）** |
| 设备树 / 端口布局 | 依赖第三方分支维护 | **OpenWrt 官方自带**，本工程不再维护 DTS |
| 风扇管理 | `luci-app-Airpifanctrl`（从第三方仓库拷进 `package/`） | `luci-app-h5000m-fancontrol`（+ 设备树补丁） |
| 出口优先级 | `mwan3`（已在上游流程中移除） | `luci-app-h5000m-netmode` |
| 5G 拨号 | QModem / `luci-app-modem` + ModemManager | `ddimension/wwand`（原生 QMI/MBIM/NCM/MHI） |
| 包管理器 | opkg | apk（OpenWrt 25.12 起） |
...
（OpenWrt、LuCI、wwand 为 GPL-2.0-only，`luci-app-h5000m-*` 为 Apache-2.0）。
